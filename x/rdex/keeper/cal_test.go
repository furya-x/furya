...
import (
	...
	rdexKeeper "github.com/warmage-sports/katana/x/rdex/keeper"
...
func TestCalPoolUnitV2(t *testing.T) {
	testcases := []struct {
		name                 string
		oldPoolUnits         sdk.Int
		nativeAssetBalance   sdk.Int
		...
		{
			name:                 "successful4",
			oldPoolUnits:         stringToInt("1692870066"),
			nativeAssetBalance:   stringToInt("23764651856"),
			externalAssetBalance: stringToInt("34375161733"),
			nativeAssetAmount:    stringToInt("691332100"),
			externalAssetAmount:  stringToInt("1000000000"),
			lpunits:              stringToInt("803080648314941877218"),
			poolUnits:            stringToInt("803080648314941877218").Mul(sdk.NewInt(2)),
		},
		{
...
func TestCalcSwapV2(t *testing.T) {
	testcases := []struct {
		name            string
		inputIsBase     bool
		Y, X, x, y, fee sdk.Int
		...
		{
			Y:           stringToInt("705982027"),
			X:           stringToInt("12810561677"),
			x:           stringToInt("10000000"),
			inputIsBase: true,
			y:           sdk.NewInt(0),
			fee:         sdk.NewInt(0),
		},
		{
			Y:           sdk.NewInt(24386850746 - 622198870),
			X:           sdk.NewInt(35275161728 - 900000005),
			x:           sdk.NewInt(900000005),
			inputIsBase: false,
			y:           sdk.NewInt(0),
			fee:         sdk.NewInt(0),
		},
...
func TestCalRemoveAmountV2(t *testing.T) {
	testcases := []struct {
		name                    string
		poolUnits               sdk.Int
		rmUnits                 sdk.Int
		...
		{
			poolUnits:               sdk.NewInt(1737192274),
			rmUnits:                 sdk.NewInt(44322208),
			swapUnits:               sdk.NewInt(44322208),
			nativeAssetBalance:      sdk.NewInt(24386850746),
			externalAssetBalance:    sdk.NewInt(35275161728),
			inputIsBase:             false,
			expectRmBaseTokenAmount: sdk.NewInt(2),
			expectRmTokenAmount:     sdk.NewInt(2),
			expectSwapAmount:        sdk.NewInt(2),
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			baseTokenAmount, tokenAMount, swapAmount := rdexKeeper.CalRemoveAmount(tc.poolUnits, tc.rmUnits, tc.swapUnits, tc.nativeAssetBalance, tc.externalAssetBalance, tc.inputIsBase)
			t.Log(baseTokenAmount, tokenAMount, swapAmount)
		})
	}
}
