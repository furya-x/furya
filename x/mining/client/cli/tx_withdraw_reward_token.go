...
func CmdWithdrawRewardToken() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "withdraw-reward-token [stake-pool-index] [reward-pool-index] [withdraw-amount]",
		Short: "Withdraw reward token",
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			if !ok {
				return fmt.Errorf("argWithdrawAmount parse failed")
			}
			...
			msg := types.NewMsgWithdrawRewardToken(
